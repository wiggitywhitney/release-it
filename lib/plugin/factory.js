...
export let getPlugins = async (config, container) => {
  return tracer.startActiveSpan('release_it.plugin.get_plugins', async span => {
    try {
      ...
      span.setAttribute('release_it.plugins.enabled_count', enabledPlugins.length);
      span.setAttribute('release_it.plugins.external_count', enabledExternalPlugins.length);
