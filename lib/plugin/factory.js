...
const load = async pluginName => {
  return tracer.startActiveSpan('release_it.plugin.load', async span => {
    span.setAttribute('release_it.plugin.namespace', pluginName);
    try {
      let plugin;
      try {
...
export let getPlugins = async (config, container) => {
  return tracer.startActiveSpan('release_it.plugin.get_plugins', async span => {
    try {
      ...
      span.setAttribute('release_it.plugin.enabled_count', enabledPlugins.length);
      span.setAttribute('release_it.plugin.external_count', enabledExternalPlugins.length);
