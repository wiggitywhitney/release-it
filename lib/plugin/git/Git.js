...
async function isGitRepo() {
  return tracer.startActiveSpan('release_it.git.is_git_repo', async span => {
    try {
      ...
      span.setAttribute('release_it.git.is_repo', result);
      return result;
    } catch (error) {
...
class Git extends GitBase {
  ...
  static async isEnabled(options) {
    return tracer.startActiveSpan('release_it.git.is_enabled', async span => {
      try {
        const result = options !== false && (await isGitRepo());
        span.setAttribute('release_it.git.enabled', result);
        return result;
      } catch (error) {
        span.recordException(error);
        span.setStatus({ code: SpanStatusCode.ERROR });
  ...
  async beforeRelease() {
    return tracer.startActiveSpan('release_it.git.before_release', async span => {
      try {
        if (this.options.commitMessage != null) {
          span.setAttribute('release_it.git.commit_message', this.options.commitMessage);
        }
  ...
  async isRequiredBranch() {
    return tracer.startActiveSpan('release_it.git.is_required_branch', async span => {
      try {
        const branch = await this.getBranchName();
        if (branch != null) {
          span.setAttribute('vcs.ref.head.name', branch);
        }
  ...
  async hasUpstreamBranch() {
    return tracer.startActiveSpan('release_it.git.has_upstream_branch', async span => {
      try {
        ...
        const result = Boolean(branch);
        span.setAttribute('release_it.git.has_upstream', result);
        return result;
      } catch (error) {
        span.recordException(error);
        span.setStatus({ code: SpanStatusCode.ERROR });
  ...
  async getUpstreamArgs(pushRepo) {
    return tracer.startActiveSpan('release_it.git.get_upstream_args', async span => {
      try {
        if (pushRepo != null) {
          span.setAttribute('release_it.git.push_repo', pushRepo);
        }
  ...
  async rollbackTagPush() {
    return tracer.startActiveSpan('release_it.git.rollback_tag_push', async span => {
      try {
        const { isTagged } = this.getContext();
        if (isTagged) {
          const { tagName } = this.config.getContext();
          span.setAttribute('release_it.git.tag_name', tagName);
