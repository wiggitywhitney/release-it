...
class Version extends Plugin {
  ...
  async getIncrementedVersion(options) {
    return tracer.startActiveSpan('release_it.version.get_incremented_version', async span => {
      try {
        const { isCI } = this.config;
        span.setAttribute('release_it.is_ci', isCI);
        if (options.latestVersion != null) {
          span.setAttribute('release_it.version.current', options.latestVersion);
        }
        if (options.increment != null) {
          span.setAttribute('release_it.version.increment', options.increment);
        }
        const version = this.incrementVersion(options);
        const result = version || (isCI ? null : await this.promptIncrementVersion(options));
        if (result != null) {
          span.setAttribute('release_it.version.next', String(result));
        }
        return result;
      } catch (error) {
