  ...
  async init() {
    return tracer.startActiveSpan('release_it.config.init', async span => {
      try {
        ...
        span.setAttribute('release_it.is_dry_run', Boolean(this._options['dry-run']));
        span.setAttribute('release_it.is_ci', Boolean(this._options.ci));
      } catch (error) {
...
async function loadOptions(constructorConfig) {
  return tracer.startActiveSpan('release_it.config.load_options', async span => {
    try {
      ...
      span.setAttribute('release_it.is_ci', Boolean(expanded.ci));
      span.setAttribute('release_it.is_dry_run', Boolean(expanded['dry-run']));
      if (expanded.version != null && expanded.version.increment != null) {
        span.setAttribute('release_it.version.increment', expanded.version.increment);
      }
      return {
        options: expanded,
        localConfig
...
async function loadLocalConfig(constructorConfig) {
  return tracer.startActiveSpan('release_it.config.load_local_config', async span => {
    try {
      ...
      span.setAttribute('release_it.config.file', String(file));
